(ns hacking-capital.oanda
  (:require [clj-http.client :as client]
            [clojure.string :as str]
            [clojure.instant :as instant]
            [hacking-capital.utils :as utils]
            [clojure.edn :as edn]
            ;; my staff
  ...
  ;;(:use
   ;;[];; [hacking-capital
        ;;  [fudi :only [send-udp send-tcp]]
        ;;  [heikin :refer :all]
        ;;  [date :refer :all]
        ;;  [process-csv :only [mean variance]]]
    ;;    :reload-all)
)
...
(defn auth
  []
  (try (slurp (System/getProperty "auth"))
       (catch Exception e
         {})))
...
(defn candle->vec-0-1
  [candle stat]
  (let [prices (filter #(instance? java.lang.Double %) (vals candle))]
    (map #(utils/->0-1 % (:min stat) (:max stat)) prices ))
  )
